...
const Center: React.FC = () => {
  ...
  const [isExportingCSV, setIsExportCSV] = useState<boolean | undefined>(
    undefined
  );
  ...
  const handleExportCSV = async () => {
    ...
    downloadCSV(csv, "batch_list_export.csv");
  };
  ...
  const handleActionForDelete = async () => {
    if (selectedCohortId) {
      ...
      if (resp?.responseCode === 200) {
        showToastMessage(t("BATCHES.CENTER_DELETE_SUCCESSFULLY"), "success");
  ...
  const handleUpdateAction = async (
    ...
    try {
      ...
      if (!selectedCohortId) {
        showToastMessage(t("BATCHES.NO_COHORT_ID_SELECTED"), "error");
        return;
      }
      const cohortDetails = {
      ...
      if (resp?.responseCode === 200 || resp?.responseCode === 201) {
        showToastMessage(t("BATCHES.CENTER_UPDATE_SUCCESSFULLY"), "success");
        ...
        showToastMessage(t("BATCHES.CENTER_UPDATE_FAILED"), "error");
      }
    } catch (error) {
      ...
      } else showToastMessage(t("BATCHES.CENTER_UPDATE_FAILED"), "error");
    } finally {
      setLoading(false);
      setConfirmButtonDisable(false);
      onCloseEditMOdel();
  ...
  const userProps = {
    userType: t("SIDEBAR.BATCHES"),
    searchPlaceHolder: t("BATCHES.SEARCHBAR_PLACEHOLDER"),
        ...
        <ConfirmationModal
          message={
            selectedRowData?.totalActiveMembers > 0
              ? t("BATCHES.YOU_CANT_DELETE_CENTER_HAS_ACTIVE_LEARNERS", {
                  activeMembers: `${selectedRowData?.totalActiveMembers}`,
                })
              : t("BATCHES.SURE_DELETE_CENTER") +
        ...
        <HeaderComponent {...userProps}>
          ...
          {loading ? (
            ...
            <KaTableComponent
              ...
              limit={pageLimit}
              offset={pageOffset}
              paginationEnable={totalCount > Numbers.ONE}
