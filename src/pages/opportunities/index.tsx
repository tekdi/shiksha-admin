...
export default function OpportunitiesPage() {
  ...
  const { page = "1", search = "", industry, skills, category, status, location } = router.query
  ...
  function handleSearch(term: string) {
    const query = { ...router.query, search: term, page: "1" } as {search?: string, page: string}
    ...
    <>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        <Box sx={{ mb: 4 }}>
          ...
          >
            <SearchInput placeholder={t('OPPORTUNITY.SEARCH_OPPORTUNITIES')} defaultValue={search as string} onSearch={handleSearch} />
        ...
        <Paper elevation={2} sx={{ mb: 3, overflow: "hidden" }}>
          {isLoading ? (
            ...
            <OpportunitiesList
              ...
              onView={(opportunity:any) => router.push(`/opportunities/${opportunity.id}`)}
            />
          )}
        </Paper>
